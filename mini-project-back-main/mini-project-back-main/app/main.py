...
async def analyze_photo(image: UploadFile = File(...)):
    ...
    prompt = f"""
    The following is a JSON-formatted result analyzing whether the user's facial photo meets the passport photo guidelines.
    Based on this result, please determine if the photo is suitable, and write user-friendly advice in a kind and easy-to-understand tone.

    Conditions:

    Let the user know whether the photo is overall suitable or not.

    If it is not suitable, provide photo-taking tips on what could be improved.

    Write in a tone that is soft, friendly, and not too formal.
