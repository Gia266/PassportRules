def analyze_face(image_path):
    ...
    try:
        ...
        return {"error": f"DeepFace indeterminable: {str(e)}"}
    ...
    cheek_diff = None
    lip_asymmetry = None
    eyebrow_hidden = "Inconclusive"
    ear_hidden = "Inconclusive"
    gaze_result = "Inconclusive"
    face_straight = "Inconclusive"
    center_offset = None
    ...
    if mediapipe_result.multi_face_landmarks:
        ...
        eyebrow_hidden = "⭕ Visible" if avg_eyebrow_diff >= 0.2 else "❌ Obstructed"

        # 귀 노출
        face_width = abs(get_x(454) - get_x(234))
        ear_hidden = "⭕ Visible" if face_width >= (w * 0.30) else "❌ Obstructed"
        ...
        gaze_result = (
            "⭕ Centred" if gaze_left < 0.1 and gaze_right < 0.1 else "❌ Offset"
        )
        ...
        face_straight = "⭕ Straight" if nose_offset < (face_width * 0.03) else "❌ Askew"
        ...
        if (
            ...
            and (eyebrow_hidden == "⭕ Visible")
            and (ear_hidden == "⭕ Visible")
            and (gaze_result == "⭕ Centred")
            and (face_straight == "⭕ Straight")
        ):
            judgment = "⭕ Pass"
        else:
            judgment = "❌ Fail"
